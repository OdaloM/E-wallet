...
@Composable
fun SendMoneytpreview(){
    SendMoney(rememberNavController())
...
class PaymentScreen(navController: NavHostController) : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val layout = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            val textView = TextView(this@PaymentScreen).apply {
                text = "Send Money"
                textSize = 24f
                gravity = Gravity.CENTER
            }
            addView(textView)
            val editTextAmount = EditText(this@PaymentScreen).apply {
                hint = "Enter amount"
                inputType = InputType.TYPE_NUMBER_FLAG_DECIMAL
            }
            addView(editTextAmount)
            val editTextRecipient = EditText(this@PaymentScreen).apply {
                hint = "Enter recipient's email or phone number"
                inputType = InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS
            }
            addView(editTextRecipient)
            val buttonPay = Button(this@PaymentScreen).apply {
                text = "Pay"
                setOnClickListener {
                    // Handle payment logic here
                    Toast.makeText(this@PaymentScreen, "Payment successful!", Toast.LENGTH_SHORT).show()
                }
            }
            addView(buttonPay)
        }
